...
// Returns a new rectangle that is the interdiv of the two rectangles. If they don't intersect, returns false
...
// Computes the interdiv of the two ranges. Will return fresh date clones in a range.
// Returns undefined if no interdiv.
// Expects all dates to be normalized to the same timezone beforehand.
// TODO: move to date div?
...
var CoordCache = FC.CoordCache = Class.extend({
	...
	// If no interdiv is made, returns undefined.
			...
			if (subjectEl && this.options.subjectCenter) {
				...
				if (this.origHit) {
					subjectRect = intersectRects(this.origHit, subjectRect) ||
						subjectRect; // in case there is no interdiv
				}
...
var DayTableMixin = FC.DayTableMixin = {
	...
	sliceRangeByRow: function(range) {
		...
		for (row = 0; row < this.rowCnt; row++) {
			...
			if (segFirst <= segLast) { // was there any interdiv with the current row?
				segs.push({
					row: row,
...
DayGrid.mixin({
	...
	resliceDaySegs: function(segs, dayDate) {
		...
		segs = this.eventsToSegs(
			events,
			function(range) {
				var seg = intersectRanges(range, dayRange); // undefind if no interdiv
...
function Toolbar(calendar, toolbarOptions) {
	...
	function render() {
		var divs = toolbarOptions.layout;
		...
		if (divs) {
			...
			el.append(renderdiv('left'))
				.append(renderdiv('right'))
				.append(renderdiv('center'))
	...
	function renderdiv(position) {
		var divEl = $('<div class="fc-' + position + '"/>');
		...
		if (buttonStr) {
			$.each(buttonStr.split(' '), function(i) {
				...
				if (groupChildren.length > 1) {
					...
					divEl.append(groupEl);
				}
				else {
					divEl.append(groupChildren); // 1 or 0 children
				}
			});
		}
...
Calendar.prototype.isSpanAllowed = function(span, constraint, overlap, event) {
	...
	for (i = 0; i < peerEvents.length; i++)  {
		...
		// there needs to be an actual interdiv before disallowing anything
